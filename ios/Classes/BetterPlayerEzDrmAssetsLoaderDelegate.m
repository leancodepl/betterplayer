 ...
 **
 ** Takes the bundled SPC and sends it to the license server defined at
 *licenseUrl or KEY_SERVER_URL (if licenseUrl is null).
 ** It returns CKC.
 ** ---------------------------------------*/
- (NSData *)
    getContentKeyAndLeaseExpiryFromKeyServerModuleWithRequest:
  ...
  NSURL *ksmURL = [[NSURL alloc]
      initWithString:[NSString stringWithFormat:@"%@%@%@", finalLicenseURL,
                                                assetId, customParams]];

  NSMutableURLRequest *request =
      [[NSMutableURLRequest alloc] initWithURL:ksmURL];
  ...
  @try {
    decodedData = [NSURLConnection sendSynchronousRequest:request
...
- (BOOL)resourceLoader:(AVAssetResourceLoader *)resourceLoader
  ...
  try {
    requestBytes = [loadingRequest
        streamingContentKeyRequestDataForApp:certificate
                           contentIdentifier:
  ...
  NSString *passthruParams =
      [NSString stringWithFormat:@"?customdata=%@", _assetId];
  NSData *responseData;
  NSError *error;

  responseData = [self
      getContentKeyAndLeaseExpiryFromKeyServerModuleWithRequest:requestBytes
                                                            and:_assetId
                                                            and:passthruParams
                                                            and:error];
